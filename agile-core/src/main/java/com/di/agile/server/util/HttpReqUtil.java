...
public class HttpReqUtil {
	public static Object getVal(Parameter p, Map<String, Object[]> reqs) {
		...
		if (type == byte.class || type == Byte.class) {
			...
			if (vs.length > 0) {
				...
				if (type2 == java.lang.String.class) {
					...
					for (int i = 0; i < vs.length; i++) {
						os.add(Short.valueOf((String) vs[i]));
					}
				} else if (type2 == int.class || type2 == Integer.class) {
					for (int i = 0; i < vs.length; i++) {
						os.add(Integer.valueOf((String) vs[i]));
					}
				} else if (type2 == long.class || type2 == Long.class) {
					for (int i = 0; i < vs.length; i++) {
						os.add(Long.valueOf((String) vs[i]));
					}
				} else if (type2 == float.class || type2 == Float.class) {
					for (int i = 0; i < vs.length; i++) {
						os.add(Float.valueOf((String) vs[i]));
					}
				} else if (type2 == double.class || type2 == Double.class) {
					for (int i = 0; i < vs.length; i++) {
						os.add(Double.valueOf((String) vs[i]));
					}
				} else if (type2 == java.util.Date.class) {
					for (int i = 0; i < vs.length; i++) {
	...
	public static void setVal(Object o, String prefix, int index, Map<String, Object[]> reqs) {
		...
		for (Field f : fields) {
			...
			Object[] vs = reqs.get(n) == null ? new Object[0] : reqs.get(n);
			Class<?> type = f.getType();
			String val = (vs == null || vs.length == 0) ? "" : (String) vs[index];
			try {
				if (type == byte.class || type == Byte.class) {
					...
					} else if (genericType == com.di.agile.core.server.bean.MultipartFile.class) {
						for (int i = 0; i < vs.length; i++) {
							os.add(vs[i]);
						}
					} else {
